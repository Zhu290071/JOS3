...
# 分布系数用于在各身体部位间分配出汗速率。该分布取自Gagge等人（1971）
# 和ISO 7933中所采用的身体部位权重，用于在蒸发需求较高的情况下将
# 必要的蒸发量合理分配到全身。【Gagge et al., 1971, Journal of Applied
# Physiology 33(6); ISO 7933:2004】
_SKIN_SWEAT_DISTRIBUTION = np.array([
        0.064, 0.017, 0.146, 0.129, 0.206,
        0.051, 0.026, 0.0155, 0.051, 0.026, 0.0155,
        0.073, 0.036, 0.0175, 0.073, 0.036, 0.0175,])


def skin_sweat_distribution():
    """Return a copy of the default skin sweat distribution."""

    return _SKIN_SWEAT_DISTRIBUTION.copy()
...
def conv_coef(posture="standing", va=0.1, ta=28.8, tsk=34.0,):
    ...
    posture : str, optional
        Select posture from standing, sitting or lying.The default is "standing".
        选择姿势：站立（standing）、坐姿（sitting）或躺卧（lying）。默认是 "standing"。
    va : float or iter, optional
        Air velocity [m/s]. If iter is input, its length should be 17.The default is 0.1.
        空气流速 [m/s]。如果输入可迭代对象，其长度应为 17。默认是 0.1。
    ta : float or iter, optional
        Air temperature [oC]. If iter is input, its length should be 17.The default is 28.8.
        空气温度 [oC]。如果输入可迭代对象，其长度应为 17。默认是 28.8。
    tsk : float or iter, optional
        Skin temperature [oC]. If iter is input, its length should be 17.The default is 34.0.
        皮肤温度 [oC]。如果输入可迭代对象，其长度应为 17。默认是 34.0。
...
def operative_temp(ta, tr, hc, hr):
    # 计算操作温度（综合空气温度和辐射温度的有效温度）
    to = (hc*ta + hr*tr) / (hc + hr)
    return to


def clo_area_factor(clo):
    # 计算服装面积系数，与服装热阻 clo 相关
    fcl = np.where(clo<0.5, clo*0.2+1, clo*0.1+1.05)
    return fcl


def dry_r(hc, hr, clo, pt=101.33, clo_includes_air_layer=False):
    """
    ...
    clo_includes_air_layer : bool, optional
        When ``True`` the provided ``clo`` values are interpreted as the total
        insulation (clothing plus the boundary air layer). In that case the
        returned resistance matches the supplied value and only the portion in
        excess of the boundary layer is attributed to the clothing layer.
        默认情况下，clo 被视为固有服装热阻；若设为 True，则认为其包含
        服装与内部空气层的总热阻。
    ...
    # 计算空气层热阻
    r_cl = 0.155 * clo
    # 计算服装热阻（0.155 是 clo 到 m²·K/W 的转换系数）
    if clo_includes_air_layer:
        r_t = r_cl
        r_cl = np.maximum(r_t - r_a / fcl, 0.0)
    else:
        r_t = r_a / fcl + r_cl
    # 计算总显热热阻
    return r_t
...
def wet_r(hc,
          ...
          pt=101.33,
          ret_cl=None,
          ret_cl_includes_air_layer=False,
    ...
    ret_cl : float or array, optional
        Clothing evaporative resistance [Pa.m2/W]. If provided, this value
        is used instead of ``iclo`` to determine the clothing vapor
        resistance. Values larger than 1 are assumed to be provided in
        Pa.m2/W and will be converted to kPa.m2/W internally.
        若提供 ret_cl，则使用该服装蒸发热阻（单位：Pa·m²/W）计算湿热阻。
    ret_cl_includes_air_layer : bool, optional
        When ``True`` the supplied ``ret_cl`` values are interpreted as the
        total evaporative resistance (clothing plus the boundary air layer).
        The clothing contribution is inferred by subtracting the boundary
        layer term. Default is ``False``.
    return_components : bool, optional
        If True, returns a tuple ``(r_et, r_ea, r_ecl, fcl)``.
        若为 True，返回 ``(r_et, r_ea, r_ecl, fcl)``。
    Returns
    -------
    ret : float or array
    ...
    if ret_cl is not None:
        # ret_cl 的单位通常为 [Pa·m2/W]，转换为 [kPa·m2/W]
        r_ecl = np.asarray(ret_cl, dtype=float)
        if not np.all(np.isnan(r_ecl)):
            if np.nanmax(np.abs(r_ecl)) > 1.0:
                r_ecl = r_ecl / 1000.0
        if ret_cl_includes_air_layer:
            r_et = r_ecl
            r_ea_eff = np.minimum(r_ea / fcl, r_et)
            r_ea = r_ea_eff * fcl
            r_ecl = np.maximum(r_et - r_ea_eff, 0.0)
        else:
            r_et = r_ea / fcl + r_ecl
    else:
...
def heat_resistances(
    ...
    hcc = hc * ((pt / 101.33) ** 0.55)
    r_a = 1 / (hcc + hr)
    clo_includes_air = options.get("clo_includes_air_layer", False)
    r_cl = 0.155 * clo
    if clo_includes_air:
        r_t = r_cl
        r_cl = np.maximum(r_t - r_a / fcl, 0.0)
    ...
    # 蒸发热阻及其组成
    ret_includes_air = options.get("ret_cl_includes_air_layer", False)
    r_et, r_ea, r_ecl, _ = wet_r(
            hc,
            clo,
            iclo=iclo,
            lewis_rate=16.5,
            pt=pt,
            ret_cl=ret_cl,
            ret_cl_includes_air_layer=ret_includes_air,
...
def evaporation(err_cr, err_sk, tsk, ta, rh, ret,
    ...
    """
    wrms, clds = error_signals(err_cr, err_sk,)  # 体温调节信号
    bsar = cons.bsa_rate(height, weight, equation,)  # 体表面积比例系数
    bsa = _BSAst * bsar  # 各部位体表面积
    p_a = antoine(ta) * rh / 100  # 环境饱和蒸汽压 [kPa]
    p_sk_s = antoine(tsk)  # 皮肤表面饱和蒸汽压 [kPa]

    e_max = (p_sk_s - p_a) / ret * bsa  # 最大蒸发热损失

    skin_sweat = skin_sweat_distribution()
    ...
    wet = np.minimum(wet, 1)  # 湿润度上限为 1
    # 总蒸发热损失
    e_sk = wet * e_max
    # 有效出汗散热（扣除基础扩散）
    e_sweat = (wet - 0.06) / 0.94 * e_max

    return wet, e_sk, e_max, e_sweat


def enforce_required_evaporation(required_evap, wet, e_max,
                                 distribution=None, baseline_wet=0.06):
    """Raise skin wettedness when exercise demands additional evaporation.

    Parameters
    ----------
    required_evap : float
        Total evaporative heat loss required to maintain heat balance [W].
    wet : array_like
        Current skin wettedness estimate [-].
    e_max : array_like
        Maximum evaporative capacity by segment [W].
    distribution : array_like, optional
        Weights used to distribute the required evaporation across body
        segments.  When ``None`` the default :func:`skin_sweat_distribution`
        is used.
    baseline_wet : float, optional
        Minimum wettedness associated with regulatory dry diffusion [-].

    Returns
    -------
    wet : numpy.ndarray
        Updated skin wettedness by segment [-].
    e_sk : numpy.ndarray
        Updated total evaporative heat loss by segment [W].
    e_sweat : numpy.ndarray
        Updated evaporative heat loss attributable to sweating alone [W].

    Notes
    -----
    Exercise introduces a feed-forward sudomotor drive that raises sweating
    before core temperatures rise appreciably (Nadel et al., 1971, *Journal of
    Applied Physiology*, 31(5), 708-712).  ISO 7933 (2004) represents this
    behaviour by matching the required evaporation ``E_req`` to the available
    capacity ``E_max``.  The present helper implements the same concept by
    scaling the wettedness towards the requirement while respecting the local
    ``E_max`` limits.
    """

    wet = np.asarray(wet, dtype=float).copy()
    e_max = np.asarray(e_max, dtype=float)
    if distribution is None:
        distribution = skin_sweat_distribution()
    else:
        distribution = np.asarray(distribution, dtype=float)

    required_evap = float(max(required_evap, 0.0))
    total_weight = distribution.sum()
    if required_evap <= 0.0 or total_weight <= 0.0:
        e_sk = wet * e_max
        e_sweat = np.maximum((wet - baseline_wet) / (1.0 - baseline_wet), 0.0) * e_max
        return wet, e_sk, e_sweat

    weights = distribution / total_weight
    local_required = required_evap * weights
    safe_e_max = np.where(e_max > 0.0, e_max, np.inf)
    target_fraction = np.clip(local_required / safe_e_max, 0.0, 1.0)
    dynamic_range = max(1.0 - baseline_wet, 1e-6)
    target_wet = baseline_wet + dynamic_range * target_fraction
    wet = np.maximum(wet, target_wet)
    wet = np.clip(wet, baseline_wet, 1.0)

    e_sk = wet * e_max
    e_sweat = np.maximum((wet - baseline_wet) / dynamic_range, 0.0) * e_max
    return wet, e_sk, e_sweat
...
def ava_bloodflow(err_cr, err_sk,
    ...
    # 计算核心体温平均误差
    cap_bcr = [10.2975, 9.3935, 13.834]  # Thermal capacity at Chest, Back and Pelvis   # 胸部、背部和骨盆的热容量
...
def shivering(err_cr, err_sk, tcr, tsk,
    ...
    global PRE_SHIV  # Previous shivering thermogenesis [W] # 前一次的颤抖产热 [W]
    if options:
        if options["limit_dshiv/dt"]:
            # Asaka, 2016
            # dshiv < 0.0077 [W/s]
            # 颤抖产热变化率限制 < 0.0077 [W/s]
            dshiv = sig_shiv - PRE_SHIV
            if options["limit_dshiv/dt"] is True: # default is 0.0077 [W/s] # 默认限制为 0.0077 [W/s]
...
def sum_m(mbase, mwork, mshiv, mnst):
    ...
    qcr += mnst  # Non-shivering thermogenesis occurs in core layers    # 非颤抖产热发生在核心层
...
def get_lts(ta):
    # 计算 latent heat of vaporization（汽化潜热）[J/kg]
    return 2.418*1000